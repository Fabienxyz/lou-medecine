...
export function renderSvg(spec) {
  if (spec.intent === "process-flow") {
    return renderProcessFlowSvg(spec);
  }
  return {
    ok: false,
  ...
  for (const label of labels) {
    if (label.length > MAX_PROCESS_FLOW_LABEL_CHARS) {
      return {
        ok: false,
        errors: [
          `process-flow: step label exceeds ${MAX_PROCESS_FLOW_LABEL_CHARS} characters (${spec.element})`,
        ],
      };
    }
  }
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 638" role="img" aria-labelledby="svg-title svg-desc" data-blueprint-element="${escapeXml(spec.element)}">
  ...
  <desc id="svg-desc">${escapeXml(title)}</desc>
  <defs>
...
/** @deprecated alias */
export const renderMecOapSvg = (spec) => renderProcessFlowSvg(spec).svg;
