...
**Dernière mise à jour :** 2026-08-01 (socle modèle Offline D2-B ; objectif actif : implémentation PDR-D2)
...
| | |
|---|---|
| **Objectif actif** | [PDR-D2 — Mode hors ligne intégral](governance/PRODUCT-DECISION-REGISTRY.md) (dans le cadre [Acceptation Reader V1](MASTER_ROADMAP.md#acceptation-reader-v1)) |
...
- **Infra offline Reader minimale** — package autonome (Collège in-package), shell sans CDN, Service Worker cache-first ; fonctionnement hors ligne après warm cache ; **PDR-D2 complet ouvert** (offline intégral sur packages **installés** — prérequis D1 satisfait).
...
|---|---|---|
| **PDR-D2 — Offline complet** | [PDR-D2](governance/PRODUCT-DECISION-REGISTRY.md) · Acceptation Reader V1 | **D2-B socle livré** — modèle `offline_status` + persistance `library.json` ; prochain : Offline Manager (préparation asynchrone) |
...
| **PDR-D1 — Bibliothèque installable** | [PDR-D1](governance/PRODUCT-DECISION-REGISTRY.md) | **Clôturé** — D1-A Library Catalog Contract · D1-B identité Release · D1-C installation atomique · D1-D Package Access |
...
| Tests lou-build | **149/149** PASS | 128 JS + 21 TS (test:ci) ; 13 tests offline D2-B |
...
1. **PDR-D2 — Offline complet** — offline intégral sur packages installés ; wiring Reader ↔ Package Access (chantier actif).
...
5. **Validation pédagogique Lou** — package + Reader accepté.
...
| Date | Événement |
|---|---|
| 2026-08-01 | **D2-B — Socle modèle Offline** — `offline-state.js` : machine à états, validation transitions, persistance `offline_status` dans `library.json` ; migration legacy → `not_prepared` ; 13 tests dédiés |
...
*Révision 2026-08-01 — socle modèle Offline D2-B livré ; implémentation PDR-D2 active (Offline Manager à suivre).*
