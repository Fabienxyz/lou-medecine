...
- **Purpose.** The renderer's **index and integration contract** for the chapter package: make explicit what content exists, where to find it, in what order, understanding vs mastery, how content links to visuals and sources, which edition, what provenance, and what status.
- **Integration model (binding).** The manifest is intentionally lightweight. Each item of minimum required information below must be **available to the renderer**, either **materialised directly in the manifest** or **referenced by path to an authoritative generated sidecar** produced at packaging time (e.g. a traceability index, grounding results, or provenance carried in projection frontmatter reachable via manifest paths). Both forms are architecturally equivalent. The build declares which form applies for each item. The renderer consumes the authoritative source the manifest defines — it never recomputes, infers, or enriches package information from upstream curated artefacts.
- **Minimum required information.**
  - **Projection registry** — every projection that exists, its type, its path, and its Blueprint-element/KP references (so the renderer discovers content instead of assuming a fixed set of tabs or assets).
  - **Learning order** — pedagogy-preserving order: the Blueprint's global sequence **or** equivalent ordering declared in the manifest (e.g. projection `order` plus per-projection `elements[]`).
  - **Family tag** — each projection marked `understanding` or `mastery` (A.3).
  - **Traceability graph** — the stored `anchor ← KP ← element (where applicable) ← projection claim block` edges (Part B; the Blueprint element is present for understanding and absent for a mastery claim block grounded directly in a KP), including explicit **explanation↔visual** links by identifier (never ordinal filename). May be inline in the manifest **or** in a referenced sidecar (e.g. `build/traceability.json` via a manifest `trace_index` field).
  - **Source edition** and **provenance** stamps — inline in the manifest and/or in projection files referenced by the registry.
  - **Content status** — derived per item: coverage/reconciliation invariant PASS/FAIL, grounding-check results, and edition badges (`new` / `updated` / `unchanged` / `removed`) **derived from the KP edition history**, plus honest **known-absent** markers for planned-but-unbuilt projections. Reconciliation and grounding results may be inline **or** in referenced sidecars when the manifest declares their paths. After an edition update, the chapter also carries the **final coherence-check** result (A.2); an unchanged provenance stamp records lineage but is not by itself a currency claim.
...
- **Outputs.** One generated manifest per chapter — the renderer's **entry point** to the chapter package. Sidecars and projection files referenced by the manifest are part of the same package output.
- **Provenance.** Generated; carries the chapter's aggregate provenance, the coverage/reconciliation and grounding invariant results, and (after an edition update) the final coherence-check result — inline and/or in referenced sidecars as declared.
...
- **Purpose.** Assemble the learner experience from the chapter package; be the single place the tiers converge for a human, holding zero medical content.
- **Minimum required information (that it may rely on).** Everything C.6 makes available — whether inline in the manifest or via a manifest reference to an authoritative sidecar or projection file: the projection registry, learning order, family tags, traceability/visual links, edition badges (when derived and surfaced), known-absent markers, and Official Visual availability states. The renderer **must not** obtain this information by reading the Inventory, Blueprint, or Official Source directly.
- **References/identifiers.** Consumes IDs; defines none.
- **Inputs.** The chapter package: the **manifest** as entry point, plus any **authoritative sidecars and projection files** it references (paths resolved relative to the chapter package). Later: learner state.
