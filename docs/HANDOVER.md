...
| **Statut** | Document d'accueil — 2026-08-03 (SVG Highlight Bridge V1 clôturé ; Phase 2 prochaine) |
...
|---|---|
| **Branche** | `main` |
| **HEAD** | voir [`PROJECT_STATE.md`](PROJECT_STATE.md) — dernier jalon `svg-highlight-bridge-v1` |
...
| **Highlights SVG** | tag `svg-highlight-bridge-v1` — SVG Highlight Bridge V1 |
| **Remote** | `origin/main` — voir HEAD courant dans [`PROJECT_STATE.md`](PROJECT_STATE.md) |
| **Tags de référence** | `svg-highlight-bridge-v1` (highlights SVG) · `reader-ui-freeze-v1` (UI annotation) · `reader-acceptance-v1` (RA V1) · `display-preferences-v1` (D7) · `local-search-v1` (D6) · `offline-certification-v1` (D2) · `reader-composition-v1` (Composition) |

**Exclusions volontaires :** `_Roadmap Opus - 27 Juillet 2026.docx` · `demo/renderer/docs/learner-session-d4-technical-design.md` · scripts `demo/renderer/test/investigate/` · analyses ad hoc `docs/analysis/`
...
| | |
|---|---|
| **Phase** | Execution Mode V1 — Reader Acceptance V1 **clôturé** ; **SVG Highlight Bridge V1 clôturé** |
| **Phase opérationnelle active** | **Phase 2** — Amorçage cognitif — **prochaine** |
| **Dernier jalon publié** | **SVG Highlight Bridge V1** — highlights SVG RPC 234 ; UI annotation stabilisée HTML + SVG (2026-08-03) |
| **Objectif actif** | **Phase 2** — Amorçage cognitif (package + vue Reader) |
| **Chantier actif** | **Phase 2** — Amorçage cognitif (RPC 234) |
| **Instance courante** | Package 234 Release `complete` ; Reader V1 accepté ; 7 vues alimentées ; **highlights SVG opérationnels** sur figures inline |

**UI annotation stabilisée.** Walkthrough HTML : toolbar complète (couleurs + G/S/B + gomme). Texte SVG : couleurs + gomme uniquement (G/S/B masqués). Corrections réservées aux **bugs bloquants** — pas de lot UI ouvert.
...
**Acquis majeurs (publiés) :** gouvernance, pipeline lou-build, Composition V1, package 234, D1–D7, patrimoine E-A…E-D, Amorçage AP-A…AP-F, Reader Acceptance V1, Product Polish V1 / Annotation UI Freeze V1, **SVG Highlight Bridge V1**, modèle produit [`00-READER-V1-PRODUCT-MODEL.md`](renderer/00-READER-V1-PRODUCT-MODEL.md). **Lots D/AP clôturés** — ne pilotent plus la roadmap opérationnelle.

**Checklist implémentation :** [`docs/analysis/rpc-234-execution-audit.md`](analysis/rpc-234-execution-audit.md).

**Validations de référence :** Product Acceptance Suites [`TEST_ARCHITECTURE_V1.md`](testing/TEST_ARCHITECTURE_V1.md) · gate [`scripts/validate-reader-v1.sh`](../scripts/validate-reader-v1.sh) · Product Review manuelle · `demo/renderer/test/svg-highlight-bridge.test.js`

### Capacités Learner stabilisées (annotation — gelées HTML ; SVG highlights livrés)

| Capacité | État |
|---|---|
| Shell content-first | Gelé |
| Toolbar annotation unique | Gelée — contexte HTML / SVG |
| Highlights V2 HTML (mouseup, édition live, pas d'imbrication) | Gelés |
| **Highlights V2 SVG** (mouseup, couleur live, gomme, restauration) | **Livré** — SVG Highlight Bridge V1 |
| **Bridge Highlight V2 ↔ LouInlineFormatting** | **Livré** |
| Inline Notes HTML (création, édition, reload) | Gelées |
| Préférences indépendantes highlight / note | Gelées |
| Modèle 1 couleur + 1 style exclusif (HTML) | Gelé |
| Gomme highlight / note | Gelée |
| Édition objet entier (sélection partielle) | Gelée |
| Restauration complète après reload | Gelée |
| UI contextuelle — G/S/B masqués sur sélection SVG | Livré |
...
Learner Patrimony (E-A…E-D)
        ↓
Annotation UI — GELÉE HTML · HIGHLIGHTS SVG LIVRÉS
        ↓
Highlight V2 (HTML + SVG) → LouInlineFormatting (backend SVG)
```

**Graphical Learning Layer V1 — annotations SVG MVP :** livré (SVG Highlight Bridge V1). Conception élargie (figures/zoom/walkthrough) **non ouverte**.
...
| **Annotation UI HTML (highlights · notes · toolbar)** | **Gelée** — Annotation UI Freeze V1 |
| **Highlights SVG (Highlight V2 + LouInlineFormatting)** | **Publié** — SVG Highlight Bridge V1 |
...
|---|---|---|
| **1** | **Phase 2** — Amorçage cognitif (RPC 234) | **Prochaine** — package + vue Reader |
| — | Phase 3 — Notions | En attente |
| — | Phases 4–8 | En attente |
| — | Phase 9 — Reference Production Chapter (224) | En attente — après Product Freeze |
| — | Graphical Learning Layer V1 (conception élargie) | En attente — hors annotations MVP |

Ordre RPC = [`PROJECT_STATE.md` § Prochaines étapes](PROJECT_STATE.md#prochaines-etapes) = [`MASTER_ROADMAP.md` § Roadmap opérationnelle](MASTER_ROADMAP.md#roadmap-opérationnelle--reference-product-chapter-234).
...
1. Lire ce HANDOVER.
2. Lire [`PROJECT_STATE.md`](PROJECT_STATE.md) — **Phase 2 Amorçage cognitif prochaine** ; SVG Highlight Bridge V1 clôturé.
3. Lire [`MASTER_ROADMAP.md` § Roadmap opérationnelle](MASTER_ROADMAP.md#roadmap-opérationnelle--reference-product-chapter-234).
4. **Ne pas modifier l'UI annotation** sans instruction — gelée HTML ; highlights SVG livrés et stabilisés.
...
9. **Chantier prioritaire : Phase 2 — Amorçage cognitif** — contenu et expérience vue Amorçage sur RPC 234.
...
12. **Ne pas rouvrir** SVG Highlight Bridge V1 ni lot UI annotation sans instruction explicite.

---

*Handover — 2026-08-03 — SVG Highlight Bridge V1 clôturé ; Phase 2 Amorçage cognitif prochaine. Non normatif.*
